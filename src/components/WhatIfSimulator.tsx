    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[
          { label: t.rainfall, value: rainfall, set: setRainfall, min: 0, max: 200, color: "text-blue-500", bg: "bg-blue-500/10", unit: "mm" },
            { label: t.soilMoisture, value: soilMoisture, set: setSoilMoisture, min: 0, max: 100, color: "text-emerald-500", bg: "bg-emerald-500/10", unit: "%" },
            { label: t.windSpeed, value: windSpeed, set: setWindSpeed, min: 0, max: 120, color: "text-orange-400", bg: "bg-orange-400/10", unit: " km/h" },
            { label: t.temperature, value: temperature, set: setTemperature, min: 10, max: 50, color: "text-red-500", bg: "bg-red-500/10", unit: "°C" },
            { label: t.humidity, value: humidity, set: setHumidity, min: 0, max: 100, color: "text-cyan-500", bg: "bg-cyan-500/10", unit: "%" },
      ...
      <div className="grid grid-cols-2 gap-4">
        {[
          { label: t.rainRisk, score: rainResult.score, level: rainResult.level, colors: rainColors },
          { label: t.pestRisk, score: pestResult.score, level: pestResult.level, colors: pestColors },
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="glass-card rounded-2xl p-6">
        ...
        <div className="flex gap-4 mt-2 justify-center">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <div className="w-3 h-3 rounded-full bg-blue-500" /> {t.rainRisk}
          </div>
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <div className="w-3 h-3 rounded-full bg-amber-400" /> {t.pestRisk}
          </div>
        </div>
      </motion.div>
    </div>
