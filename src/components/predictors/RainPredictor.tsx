...
function getDemoData(query: string) {
  const demos: Record<string, { rainfall: number; windSpeed: number; humidity: number }> = {
    "Chennai": { rainfall: 12, windSpeed: 22, humidity: 88 },
    "Coimbatore": { rainfall: 5, windSpeed: 18, humidity: 72 },
    "Madurai": { rainfall: 0, windSpeed: 25, humidity: 65 },
    "Thanjavur": { rainfall: 18, windSpeed: 15, humidity: 82 },
    "Salem": { rainfall: 8, windSpeed: 20, humidity: 78 },
    "Mumbai": { rainfall: 45, windSpeed: 35, humidity: 92 },
    "Delhi": { rainfall: 2, windSpeed: 15, humidity: 55 },
    "Pune": { rainfall: 20, windSpeed: 18, humidity: 75 },
  };
  const found = Object.keys(demos).find(k => query.toLowerCase().includes(k.toLowerCase()));
  return found ? demos[found] : { rainfall: 15, windSpeed: 20, humidity: 75 };
}
...
export default function RainPredictor() {
  ...
  const applyWeather = (data: { rainfall: number; windSpeed: number; humidity?: number }, city: string) => {
    setInputs(prev => ({
      ...prev,
      rainfall: Math.max(0, Math.min(200, data.rainfall)),
      windSpeed: Math.max(0, Math.min(120, data.windSpeed)),
      soilMoisture: data.humidity ? Math.max(0, Math.min(100, Math.round(data.humidity * 0.9))) : prev.soilMoisture,
    }));
  ...
  const fetchByLocation = async (city: string) => {
    ...
    try {
      if (apiKey) {
        ...
        const data = getDemoData(city);
        applyWeather(data, city);
        setLocationMsg(`✅ Demo data loaded for ${city} (add OWM API key for live data)`);
      }
    } catch {
      const data = getDemoData(city);
      applyWeather(data, city);
      setLocationMsg(`✅ Demo data for ${city}`);
    }
      ...
      async (pos) => {
        ...
        if (apiKey) {
          try {
            ...
            const data = getDemoData("Chennai");
            applyWeather(data, `${latitude.toFixed(2)}°N, ${longitude.toFixed(2)}°E`);
          }
        } else {
          const data = getDemoData("Chennai");
          applyWeather(data, `GPS Location (${latitude.toFixed(2)}°N)`);
          setLocationMsg("✅ Demo data loaded — add OWM API key for live GPS weather");
        }
        setLocationLoading(false);
      },
      () => {
    ...
    <div className="space-y-5">
      ...
      <div className="glass-card rounded-2xl p-5 space-y-4">
        ...
        <div className="flex gap-1 bg-muted/40 rounded-xl p-1">
          {([["city", t.cityTab], ["pincode", t.pincodeTab], ["gps", t.gpsTab]] as const).map(([mode, label]) => (
            <button
              key={mode}
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  onKeyDown={e => e.key === "Enter" && locationQuery && fetchByLocation(locationQuery)}
              ...
              <Button
                ...
                size="sm"
              >
                {locationLoading ? <Loader2 size={14} className="animate-spin" /> : t.fetch}
              </Button>
            </div>
            <AnimatePresence>
              {suggestions.length > 0 && (
                ...
                >
                  {suggestions.map(s => (
                    <button
                      key={s}
                      onClick={() => { setLocationQuery(s); fetchByLocation(s); }}
        ...
        {searchMode === "pincode" && (
          <div className="flex gap-2">
            ...
            <Button
              ...
              size="sm"
            >
              {locationLoading ? <Loader2 size={14} className="animate-spin" /> : t.lookup}
            </Button>
          </div>
        )}
        ...
        {searchMode === "gps" && (
          ...
          >
            {locationLoading ? (
              <span className="flex items-center gap-2"><Loader2 size={14} className="animate-spin" /> {t.gettingLocation}</span>
            ) : (
              <span className="flex items-center gap-2"><Navigation size={14} /> {t.useMyLocation}</span>
            )}
          </Button>
        )}

        {locationMsg && (
          <p className={`text-xs font-medium ${locationMsg.startsWith("✅") ? "text-emerald-600" : "text-destructive"}`}>
            {locationMsg}
          </p>
        )}
      </div>
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Adjust sliders or use location search above</p>
            </div>
          </div>
