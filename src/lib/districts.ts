export const INDIA_DISTRICTS: Record<string, string[]> = {
  ...
  "Maharashtra": [
    ...
    "Amravati", "Sangli", "Latur", "Ahmednagar", "Satara", "Ratnagiri",
  ],
  "Karnataka": [
    "Bengaluru", "Mysuru", "Hubballi", "Mangaluru", "Belgaum", "Gulbarga",
    "Davanagere", "Bellary", "Shimoga", "Tumkur", "Raichur", "Dharwad",
  ],
  "Andhra Pradesh": [
    "Visakhapatnam", "Vijayawada", "Guntur", "Nellore", "Tirupati", "Kurnool",
    "Rajahmundry", "Kakinada", "Eluru", "Anantapur", "Kadapa", "Srikakulam",
  ],
  "Telangana": [
    "Hyderabad", "Warangal", "Nizamabad", "Karimnagar", "Khammam", "Mahbubnagar",
    "Adilabad", "Medak", "Nalgonda", "Ranga Reddy",
  ],
  "Kerala": [
    "Thiruvananthapuram", "Kochi", "Kozhikode", "Thrissur", "Kannur", "Kollam",
    "Palakkad", "Alappuzha", "Malappuram", "Kasaragod", "Kottayam", "Pathanamthitta",
  ],
  "Uttar Pradesh": [
    "Lucknow", "Kanpur", "Varanasi", "Agra", "Allahabad", "Meerut",
    "Ghaziabad", "Bareilly", "Aligarh", "Moradabad", "Noida", "Gorakhpur",
  ],
  "Gujarat": [
    "Ahmedabad", "Surat", "Vadodara", "Rajkot", "Bhavnagar", "Jamnagar",
    "Gandhinagar", "Anand", "Mehsana", "Amreli", "Junagadh", "Kutch",
  ],
  "Rajasthan": [
    "Jaipur", "Jodhpur", "Udaipur", "Kota", "Bikaner", "Ajmer",
    "Alwar", "Sikar", "Jhunjhunu", "Bhilwara", "Barmer", "Nagaur",
  ],
  "Punjab": [
    "Ludhiana", "Amritsar", "Jalandhar", "Patiala", "Bathinda", "Mohali",
    "Pathankot", "Hoshiarpur", "Moga", "Firozpur", "Gurdaspur", "Faridkot",
  ],
  "West Bengal": [
    "Kolkata", "Howrah", "Asansol", "Durgapur", "Siliguri", "Bardhaman",
    "Kharagpur", "Bankura", "Malda", "Murshidabad", "Jalpaiguri", "Purulia",
  ],
  "Madhya Pradesh": [
    "Bhopal", "Indore", "Jabalpur", "Gwalior", "Ujjain", "Sagar",
    "Dewas", "Satna", "Ratlam", "Rewa", "Chhindwara", "Mandsaur",
  ],
};

export const ALL_DISTRICTS: { district: string; state: string }[] = Object.entries(INDIA_DISTRICTS).flatMap(
...
export function getDistrictRisk(district: string, state: string): DistrictRisk {
  ...
  const level: "LOW" | "MEDIUM" | "HIGH" = overallRisk >= 70 ? "HIGH" : overallRisk >= 40 ? "MEDIUM" : "LOW";
  return { district, state, rainRisk, pestRisk, locustRisk, overallRisk, level };
}

export function getAllDistrictRisks(): DistrictRisk[] {
  return ALL_DISTRICTS.map(({ district, state }) => getDistrictRisk(district, state));
}
